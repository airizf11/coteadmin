...
type CashSummary = { totalIn: number; totalOut: number; balance: number };
...
// TODO: sama kayak yang di reports/advanced/page.tsx — worth disatuin ke
// shared lib kalau nanti dipake di tempat ke-3.
...
export default async function ReportsPage({
  ...
  const isPersonal = b.businessType === 'PERSONAL';
    ...
    <div className="flex items-start justify-between gap-4">
      ...
      {!isPersonal && (
        ...
        >
          ...
          <ArrowRight size={14} className="ml-1.5 inline-block transition-transform group-hover:translate-x-0.5" />
        </Link>
      )}
    </div>
  );

  // ── Mode PERSONAL: gak ada order, cuma arus kas ──────────────────────
  if (isPersonal) {
    let cash: CashSummary;
    let expenses: ExpenseByCategory[];
    let income: IncomeByCategory[];
    let cashTrend: CashFlowPoint[];

    try {
      const [txRes, expenseRes, incomeRes, trendRes] = await Promise.all([
        cotebek<{ data: unknown[]; meta: { summary: CashSummary } }>(`/transactions${qs}`),
      ...
      cash = txRes.meta.summary;
      expenses = expenseRes.data;
      income = incomeRes.data;
      cashTrend = trendRes.data;
    } catch (error) {
      return (
    ...
    const maxExpense = Math.max(1, ...expenses.map((e) => e.total));
    const maxIncome = Math.max(1, ...income.map((i) => i.total));
    const maxCashValue = Math.max(1, ...cashTrend.flatMap((t) => [t.totalIn, t.totalOut]));
    ...
    return (
      <div className="mx-auto w-full max-w-5xl space-y-5 px-4 pb-24 pt-5 sm:space-y-6 sm:pt-6">
          ...
          <CardHeader className="border-b border-border/60 px-4 pb-4 pt-5 sm:px-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">Uang masuk dan keluar per hari</p>
              </div>
              <div className="flex items-center gap-3 text-[10px] font-semibold text-muted-foreground">
                <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-success" />Masuk</span>
                <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-destructive" />Keluar</span>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-4 sm:p-5">
            {cashTrend.length === 0 ? (
              <div className="flex min-h-40 flex-col items-center justify-center text-center">
                ...
                <p className="mt-3 text-sm font-medium text-foreground">Belum ada data tren</p>
              </div>
            ) : (
              <div className="relative h-56 w-full overflow-hidden">
                ...
                <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-x-0 top-2 h-[calc(100%-28px)] w-full overflow-visible" aria-label="Grafik tren arus kas" role="img">
                  {cashTrend.length > 1 && (
                    <>
                      <polyline
                        points={cashTrend.map((t, i) => { const x = (i / (cashTrend.length - 1)) * 100; const y = 96 - (t.totalIn / maxCashValue) * 88; return `${x},${y}`; }).join(' ')}
                        fill="none" stroke="currentColor" strokeWidth="1.8" vectorEffect="non-scaling-stroke" className="text-success"
                      />
                      <polyline
                        points={cashTrend.map((t, i) => { const x = (i / (cashTrend.length - 1)) * 100; const y = 96 - (t.totalOut / maxCashValue) * 88; return `${x},${y}`; }).join(' ')}
                        fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="4 3" vectorEffect="non-scaling-stroke" className="text-destructive"
                      />
                    </>
                  )}
                  {cashTrend.map((t, i) => {
                    const x = cashTrend.length === 1 ? 50 : (i / (cashTrend.length - 1)) * 100;
                    return (
                      <g key={t.date}>
                        <circle cx={x} cy={96 - (t.totalIn / maxCashValue) * 88} r="1.4" className="fill-success" />
                        <circle cx={x} cy={96 - (t.totalOut / maxCashValue) * 88} r="1.2" className="fill-destructive" />
                      </g>
                    );
                  })}
                </svg>
                <div className="absolute inset-x-0 bottom-0 flex justify-between gap-2">
                  {cashTrend.filter((_, i) => { if (cashTrend.length <= 7) return true; const step = Math.ceil(cashTrend.length / 6); return i % step === 0 || i === cashTrend.length - 1; }).map((t) => (
                    <span key={t.date} className="text-[9px] text-muted-foreground">{formatDate(t.date)}</span>
                  ))}
                </div>
              </div>
            )}
        ...
        <Card className="overflow-hidden border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/60 px-4 pb-4 pt-5 sm:px-5">
            <CardTitle className="text-sm font-bold">Pemasukan Berdasarkan Kategori</CardTitle>
          </CardHeader>
                  ...
                  return (
                    <div key={i.category} className="space-y-1.5">
                      ...
                      <div className="h-2.5 w-full overflow-hidden rounded-full bg-muted">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-success/60 to-success transition-all duration-1000 ease-out"
        ...
        <Card className="overflow-hidden border-border/80 shadow-sm">
          <CardHeader className="border-b border-border/60 px-4 pb-4 pt-5">
            <CardTitle className="text-sm font-bold">Pengeluaran Berdasarkan Kategori</CardTitle>
          </CardHeader>
          <CardContent className="p-4">
            {expenses.length === 0 ? (
                ...
                {expenses.map((e) => {
                  ...
                  return (
                    <div key={e.category} className="space-y-1.5">
                      ...
                      <div className="h-2.5 w-full overflow-hidden rounded-full bg-muted">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-destructive/60 to-destructive transition-all duration-1000 ease-out"
                          style={{ width: `${Math.max(percentage, 1)}%` }}
                        />
                      </div>
                    </div>
  ...
  // ── Mode komersial (JASA/FNB/RETAIL/OTHER): laporan penjualan biasa ──
  let summary: Summary;
  ...
  const maxTrendValue = Math.max(1, ...trend.flatMap((t) => [t.revenue, t.profit]));
  const profitMargin = summary.revenue > 0 ? (summary.grossProfit / summary.revenue) * 100 : 0;
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-5 px-4 pb-24 pt-5 sm:space-y-6 sm:pt-6">
      ...
      <Card className="relative overflow-hidden border-primary/20 bg-gradient-to-br from-primary/[0.12] via-card to-card shadow-md shadow-primary/5">
        <div aria-hidden="true" className="absolute -right-16 -top-20 h-48 w-48 rounded-full bg-primary/10 blur-3xl" />
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 border-t border-border/60 pt-4">
            ...
            <div>
              ...
              <div className="mt-1 text-sm font-bold text-foreground">
                {profitMargin.toLocaleString('id-ID', { maximumFractionDigits: 1 })}%
              </div>
            </div>
          </div>
        </CardContent>
        ...
        <Card className="border-border/80 bg-card shadow-sm transition-shadow hover:shadow-md">
          <CardContent className="p-4 sm:p-5">
            <div className="flex items-center gap-2 text-chart-3">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider sm:text-xs">Total Order</span>
            </div>
            <div className="mt-3 text-base font-bold tracking-tight text-foreground sm:text-lg">
              {summary.totalOrders.toLocaleString('id-ID')}
              <span className="ml-1 text-xs font-normal text-muted-foreground">nota</span>
            </div>
          </CardContent>
        </Card>
        ...
        <CardHeader className="border-b border-border/60 px-4 pb-4 pt-5 sm:px-5">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Omzet dan laba kotor per hari</p>
            </div>
            <div className="flex items-center gap-3 text-[10px] font-semibold text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-chart-1" />Omzet</span>
              <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-chart-2" />Laba</span>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-4 sm:p-5">
          {trend.length === 0 ? (
            <div className="flex min-h-40 flex-col items-center justify-center text-center">
              ...
              <p className="mt-3 text-sm font-medium text-foreground">Belum ada data tren</p>
              <p className="mt-1 text-xs text-muted-foreground">Data penjualan akan muncul setelah ada transaksi.</p>
            </div>
          ) : (
            <>
              <div className="relative h-56 w-full overflow-hidden">
                ...
                <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-x-0 top-2 h-[calc(100%-28px)] w-full overflow-visible" aria-label="Grafik tren omzet dan laba" role="img">
                  {trend.length > 1 && (
                    <>
                      <polyline
                        points={trend.map((t, i) => { const x = (i / (trend.length - 1)) * 100; const y = 96 - (t.revenue / maxTrendValue) * 88; return `${x},${y}`; }).join(' ')}
                        fill="none" stroke="currentColor" strokeWidth="1.8" vectorEffect="non-scaling-stroke" className="text-chart-1"
                      />
                      <polyline
                        points={trend.map((t, i) => { const x = (i / (trend.length - 1)) * 100; const y = 96 - (t.profit / maxTrendValue) * 88; return `${x},${y}`; }).join(' ')}
                        fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="4 3" vectorEffect="non-scaling-stroke" className="text-chart-2"
                      />
                    </>
                  )}
                  {trend.map((t, i) => {
                    const x = trend.length === 1 ? 50 : (i / (trend.length - 1)) * 100;
                    const revenueY = 96 - (t.revenue / maxTrendValue) * 88;
                    const profitY = 96 - (t.profit / maxTrendValue) * 88;
                    return (
                      <g key={t.date}>
                        <circle cx={x} cy={revenueY} r="1.4" className="fill-chart-1" />
                        <circle cx={x} cy={profitY} r="1.2" className="fill-chart-2" />
                      </g>
                    );
                  })}
                </svg>
                <div className="absolute inset-x-0 bottom-0 flex justify-between gap-2">
                  {trend.filter((_, i) => { if (trend.length <= 7) return true; const step = Math.ceil(trend.length / 6); return i % step === 0 || i === trend.length - 1; }).map((t) => (
                    <span key={t.date} className="text-[9px] text-muted-foreground">{formatDate(t.date)}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-2 flex items-center justify-between border-t border-border/60 pt-3">
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Periode</p>
                  <p className="mt-0.5 text-xs font-medium text-foreground">{formatDate(start)} – {formatDate(end)}</p>
                </div>
      ...
      <Card className="overflow-hidden border-border/80 shadow-sm">
        <CardHeader className="border-b border-border/60 px-4 pb-4 pt-5">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Layanan dengan jumlah penjualan tertinggi</p>
        </CardHeader>
        <CardContent className="p-0">
          {topItems.length === 0 ? (
            ...
            <ul className="divide-y divide-border/70">
              {topItems.map((item, i) => (
                <li key={item.itemName} className={cn('flex items-center justify-between gap-4 px-4 py-3.5 transition-colors hover:bg-muted/40', i === 0 && 'bg-primary/[0.035]')}>
                  <div className="flex min-w-0 items-center gap-3">
                    <div className={cn('flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-xs font-bold',
                      i === 0 ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-400'
                        : i === 1 ? 'bg-slate-100 text-slate-700 dark:bg-slate-500/15 dark:text-slate-300'
                        : i === 2 ? 'bg-orange-100 text-orange-800 dark:bg-orange-500/15 dark:text-orange-400'
                        : 'bg-muted text-muted-foreground')}>
                      {i + 1}
                    </div>
                    <span className="truncate text-sm font-medium text-foreground">{item.itemName}</span>
                  </div>
                  <span className="shrink-0 rounded-lg bg-muted px-2.5 py-1 text-xs font-bold text-foreground">{item.totalSold.toLocaleString('id-ID')}x</span>
                </li>
              ))}
            </ul>
          )}
      ...
      <Card className="overflow-hidden border-border/80 shadow-sm">
        <CardHeader className="border-b border-border/60 px-4 pb-4 pt-5">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Distribusi transaksi berdasarkan metode pembayaran</p>
        </CardHeader>
        <CardContent className="p-4 sm:p-5">
          {payments.length === 0 ? (
                ...
                return (
                  <li key={p.method}>
                    <div className="mb-2 flex items-end justify-between gap-3">
                      <span className="text-sm font-semibold text-foreground">{p.method}</span>
                      <div className="shrink-0 text-right">
                        <span className="text-xs font-bold text-foreground">{p.percentage}</span>
                        <span className="ml-1.5 text-[10px] text-muted-foreground">({p.count.toLocaleString('id-ID')}x)</span>
                      </div>
                    </div>
                    <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                      <div className="h-full rounded-full bg-chart-2 transition-all duration-700 ease-out" style={{ width: `${Math.min(Math.max(pctValue, 0), 100)}%` }} />
                    </div>
                  </li>
                );
              })}
